...
def sum_numbers_in_list(string_list: list):
    ...
    # if string_list is not list:
    #     print('twst', string_list)
    result = []
    if type(string_list) != list or (string_list == []):
        raise ValueError("Ваші дані не є списком")
    for item in string_list:
        try:
            generated_list = [int(x) for x in item.split(",")]
            result.append(sum(generated_list))
